...
function isDeveloperMode(): bool {
	// First check if DEVMODE is explicitly set to true in .env
	if (getenv('DEVMODE') === 'true') {
		return true;
	}
	
	// If no user is logged in, use DEVMODE from .env or default to false
	if (!isset($_SESSION['user_id'])) {
		return getenv('DEVMODE') === 'true';
	}
	...
	try {
		...
		// If database error, fall back to DEVMODE from .env
		return getenv('DEVMODE') === 'true';
	}
}
